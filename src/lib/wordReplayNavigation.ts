...
/** Autoplay is allowed only for one reviewed word, with no competing occurrence. */
export function wordTapDecision(result: ReturnType<typeof resolveWordReplay>) {
  const words = [...result.reviewedWords, ...result.approximateWords];
  if (words.length === 1 && words[0].status === "reviewed") {
    return { kind: "play" as const, entry: words[0] };
  }
  if (words.length > 1) return { kind: "choose" as const };
  if (words.length) return { kind: "approximate" as const };
  if (result.reviewedAyahSpans.length) return { kind: "ayah" as const };
