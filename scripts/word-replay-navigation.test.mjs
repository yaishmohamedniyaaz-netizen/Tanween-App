...
import { resolveWordReplay, wordTapDecision, makeReplayNavigationRequest, ReplayNavigationController } from "../src/lib/wordReplayNavigation.ts";
...
test("replay layout declares a reserved dock and accessible controls (browser geometry checked separately)", () => {
  const css = readFileSync(new URL("../src/styles/participantReviewWorkspace.css", import.meta.url), "utf8");
  const layout = readFileSync(new URL("../src/styles/replayPlayerLayout.css", import.meta.url), "utf8");
  const player = readFileSync(new URL("../src/components/RecitationReplayPlayer.tsx", import.meta.url), "utf8");
  assert.doesNotMatch(css, /height: 288px/);
  assert.match(layout, /grid-template-rows: auto minmax\(0, 1fr\) auto/);
  assert.match(css, /\.rw-sidebar \.rw-replay-panel \.replay-transport input \{ width: 100%; min-height: 44px;/);
  assert.match(css, /button\[aria-pressed="true"\]/);
  assert.match(player, /aria-valuetext=/);
  assert.match(player, /onCancelNavigation=\{\(\) => \{ cancelNavigation\(\); setContinuationNotice\(""\); \}\}/);
});
test("interaction wiring keeps cancellation, load states and neutral word actions explicit", () => {
  ...
  assert.match(player, /\[context.playbackRequest, context.wordReplayEnabled\]/);
  assert.match(player, /setDirectSelection\(\{ wordId: null, id: "" \}\)/);
  assert.match(player, /sourceLoading \? <p role="status">Loading recording/);
  assert.match(player, /sourceId !== loadSourceId/);
  assert.match(player, /Retry recording/);
  assert.match(player, /Preview approximate position/);
  assert.match(player, /Close word replay choices/);
  assert.match(player, /aria-live="off"/);
  assert.match(player, /ArrowRight: 1, ArrowUp: 1, ArrowLeft: -1, ArrowDown: -1, PageUp: 5, PageDown: -5/);
  assert.match(workspace, /Word replay \{replayMode/);
  assert.match(evidence, /wordReplayMode \? "Replay from" : "Inspect word"/);
});
test("word taps autoplay only one reviewed occurrence, never an uncertain fallback",()=>{
  assert.equal(wordTapDecision(resolve([entry()])).kind,"play");
  const suggested=entry({id:"r2",occurrenceId:"o2",status:"suggested"});
  assert.equal(wordTapDecision(resolve([suggested])).kind,"approximate");
  assert.equal(wordTapDecision(resolve([entry(),suggested])).kind,"choose");
  assert.equal(wordTapDecision(resolve([entry(),entry({id:"r2",occurrenceId:"o2"})])).kind,"choose");
  assert.equal(wordTapDecision(resolve([entry({target:{kind:"ayah",wordIds:["112.1.0"],label:"span"}})])).kind,"ayah");
  assert.equal(wordTapDecision(resolve([])).kind,"unavailable");
});
