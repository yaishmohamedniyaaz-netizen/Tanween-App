...
for (const entry of entries) {
  const response = await fetch(new URL(entry.url, origin), { cache: 'reload' });
  ...
  console.log(`${valid ? 'PASS' : 'FAIL'} ${entry.url} (${response.status}, ${bytes.length} bytes)`);
  if (!valid) failed = true;
  if (entry.url === '/') {
    console.log('HTML cache policy:', response.headers.get('cache-control'));
    if (!response.headers.get('cache-control')?.split(',').some(value => value.trim().toLowerCase() === 'no-transform')) failed = true;
  }
}
